 ...
 */
export async function downloadAudioViaSSYoutube(
  videoId: string
): Promise<Buffer> {
 ...
 */
export async function downloadAudioVia9Convert(
  videoId: string
): Promise<Buffer> {
 ...
 */
export async function downloadAudioViaSaveTube(
  videoId: string
): Promise<Buffer> {
  ...
  console.log(
    `[SaveTube] Downloading audio (${bestAudio.quality || "unknown"})...`
  );
 ...
 */
export async function downloadAudioViaLoaderTo(
  videoId: string
): Promise<Buffer> {
  ...
  while (attempts < maxAttempts) {
    const progressResponse = await fetch(
      `https://loader.to/ajax/progress.php?id=${initData.id}`,
      {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
...
export async function downloadAudioViaRapidAPI(
  ...
  while (attempts < maxAttempts) {
    ...
    if (data.status === "ok" && data.link) {
      ...
      if (!mp3Response.ok) {
        console.log(
          `[RapidAPI] MP3 download failed with ${mp3Response.status}, trying redirect...`
        );
        ...
        if (location) {
          ...
          if (redirectResponse.ok) {
            ...
            console.log(
              `[RapidAPI] ✅ Downloaded ${arrayBuffer.byteLength} bytes (via redirect)`
            );
            return Buffer.from(arrayBuffer);
          }
        }
...
export async function downloadYouTubeAudio(videoId: string): Promise<Buffer> {
  ...
  if (RAPIDAPI_KEY) {
    methods.unshift({
      name: "RapidAPI",
      fn: () => downloadAudioViaRapidAPI(videoId),
    });
  }
  ...
  throw new Error(
    "All download methods failed. Please try again later or use a different video."
  );
}
