 ...
 */
export async function downloadAudioViaRapidAPI(videoId: string): Promise<Buffer> {
  if (!RAPIDAPI_KEY) {
  ...
  while (attempts < maxAttempts) {
    ...
    if (data.status === 'fail') {
      throw new Error(`RapidAPI conversion failed: ${data.msg || 'Unknown error'}`);
    }
...
export async function downloadAudioViaY2mate(videoId: string): Promise<Buffer> {
  ...
  if (analyzeData.status !== 'ok') {
    throw new Error(`Y2mate analyze error: ${analyzeData.mess || 'Unknown error'}`);
  }
  ...
  if (convertData.status !== 'ok' || !convertData.dlink) {
    throw new Error(`Y2mate conversion error: ${convertData.mess || 'No download link'}`);
  }
...
export async function downloadYouTubeAudio(videoId: string): Promise<Buffer> {
  ...
  for (const method of methods) {
    try {
      ...
      console.error(`[Download] ${method.name} failed:`, error instanceof Error ? error.message : error);
    }
  }
